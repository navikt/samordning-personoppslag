...
class KafkaConfig(
    ...
    fun aivenKafkaConsumerFactory(): ConsumerFactory<Any, Any?> {
        val configMap: MutableMap<String, Any> = HashMap()
        aivenCommonConfig(configMap)
        commonConfig(aivenBootstrapServers, aivenGroupid, configMap)

        val avroValueDes = KafkaAvroDeserializer()
        val avroKeyDes = KafkaAvroDeserializer().apply {
            configure(configMap, false)
        }

        return DefaultKafkaConsumerFactory(configMap, avroKeyDes, avroValueDes)
    }


    @Bean
    fun aivenKafkaListenerContainerFactory(): ConcurrentKafkaListenerContainerFactory<String, Personhendelse> {
